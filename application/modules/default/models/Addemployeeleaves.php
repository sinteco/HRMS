	...
	*/
    public function getEmployeesData($sort,$by,$pageNo,$perPage,$searchQuery,$managerid='',$loginUserId)
    {
        ...
        $employeesData = $this->select()
                                ...
                                ->joinLeft(array('el'=>'main_employeeleaves'), 'el.user_id=e.user_id',array('el.used_leaves','ROUND(IF(((DATEDIFF(now(),e.date_of_joining)/30))>=12,20,((DATEDIFF(now(),e.date_of_joining)/30)*(20/12)))+(DATEDIFF(now(),e.date_of_joining)/365.25),2) as emp_leave_limit','ROUND((IF(((DATEDIFF(now(),e.date_of_joining)/30))>=12,20,((DATEDIFF(now(),e.date_of_joining)/30)*(20/12)))+(DATEDIFF(now(),e.date_of_joining)/365.25))/12 * MONTH(CURRENT_TIMESTAMP),2) as leaveX','el.alloted_year','el.createddate','el.isleavetrasnferset','remainingleaves'=>new Zend_Db_Expr('ROUND(((IF(((DATEDIFF(now(),e.date_of_joining)/30))>=12,20,((DATEDIFF(now(),e.date_of_joining)/30)*(20/12)))+(DATEDIFF(now(),e.date_of_joining)/365.25))/12 * MONTH(CURRENT_TIMESTAMP)),2) - el.used_leaves')))                                        
                                ->where($where)
                                ->order("$by $sort") 
    ...
    {		
        ...
        $tableFields = array('action'=>'Action','firstname'=>'First Name','lastname'=>'Last Name',
                             'employeeId' =>'Employee ID','emp_leave_limit'=>'Allotted Leave Limit','leaveX'=>'leaveX',
