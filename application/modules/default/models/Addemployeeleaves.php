	...
	*/
    public function getEmployeesData($sort,$by,$pageNo,$perPage,$searchQuery,$managerid='',$loginUserId)
    {
        ...
        $employeesData = $this->select()
                                ...
                                ->joinLeft(array('el'=>'main_employeeleaves'), 'el.user_id=e.user_id',array('el.used_leaves','ROUND(IF(((DATEDIFF(now(),e.date_of_joining)/30))>=12,20,((DATEDIFF(now(),e.date_of_joining)/30)*(20/12)))+(DATEDIFF(now(),e.date_of_joining)/365.25)-1,2) as emp_leave_limit','ROUND((DATEDIFF(now(),e.date_of_joining)/365.25),2) as serviceyear','ROUND((IF(((DATEDIFF(now(),e.date_of_joining)/30))>=12,20,((DATEDIFF(now(),e.date_of_joining)/30)*(20/12)))+(DATEDIFF(now(),e.date_of_joining)/365.25))/12 * MONTH(CURRENT_TIMESTAMP)-1,2) as leaveX','DATE_FORMAT(CURRENT_TIMESTAMP,  "%Y" ) as alloted_year','IF(el.alloted_year<year(now()),(IF(month(now())<=3, IF((ROUND(((IF(((DATEDIFF(now() - INTERVAL 1 YEAR,e.date_of_joining)/30))>=12,20,((DATEDIFF(now() - INTERVAL 1 YEAR,e.date_of_joining)/30)*(20/12)))+(DATEDIFF(now() - INTERVAL 1 YEAR,e.date_of_joining)/365.25))/12 * MONTH(CURRENT_TIMESTAMP))-1,2)) - el.used_leaves > 10,10,(ROUND(((IF(((DATEDIFF(now() - INTERVAL 1 YEAR,e.date_of_joining)/30))>=12,20,((DATEDIFF(now() - INTERVAL 1 YEAR,e.date_of_joining)/30)*(20/12)))+(DATEDIFF(now() - INTERVAL 1 YEAR,e.date_of_joining)/365.25))/12 * MONTH(CURRENT_TIMESTAMP))-1,2)) - el.used_leaves), 0)),0) as transfer','el.createddate','el.isleavetrasnferset','remainingleaves'=>new Zend_Db_Expr('ROUND(((IF(((DATEDIFF(now(),e.date_of_joining)/30))>=12,20,((DATEDIFF(now(),e.date_of_joining)/30)*(20/12)))+(DATEDIFF(now(),e.date_of_joining)/365.25))/12 * MONTH(CURRENT_TIMESTAMP))-1,2) - el.used_leaves')))                                        
                                ->where($where)
                                ->order("$by $sort") 
