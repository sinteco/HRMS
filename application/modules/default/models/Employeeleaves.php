...
class Default_Model_Employeeleaves extends Zend_Db_Table_Abstract
{
	...
	public function getEmpLeavesData($sort, $by, $pageNo, $perPage,$searchQuery,$id)
	{
		...
		$empskillsData = $this->select()
						   ...
						   ->from(array('e' => 'main_employeeleaves'),array('id'=>'e.id','emp_leave_limit'=>'IF(((DATEDIFF(now(),u.date_of_joining)/30))>=12,20,((DATEDIFF(now(),u.date_of_joining)/30)*(20/12)))+IF(FLOOR((DATEDIFF(now(),u.date_of_joining)/30)/12)>=14,14,FLOOR((DATEDIFF(now(),u.date_of_joining)/30)/12))','used_leaves'=>'e.used_leaves','remainingleaves'=>new Zend_Db_Expr('IF(((DATEDIFF(now(),u.date_of_joining)/30))>=12,20,((DATEDIFF(now(),u.date_of_joining)/30)*(20/12)))+IF(FLOOR((DATEDIFF(now(),u.date_of_joining)/30)/12)>=14,14,FLOOR((DATEDIFF(now(),u.date_of_joining)/30)/12)) - e.used_leaves'),'e.alloted_year'))
