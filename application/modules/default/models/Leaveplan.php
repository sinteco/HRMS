...
class Default_Model_Leaveplan extends Zend_Db_Table_Abstract
{
    protected $_name = 'main_leaveplan';
	...
	public function getsinglePendingLeavesData($id)
	{
		$result =  $this->select()
    				->setIntegrityCheck(false) 	
    				->from(array('l'=>'main_leaveplan'),array('l.*'))
	...
	public function getUserApprovedOrPendingLeavesData($id)
	{
		...
		$query = "SELECT `l`.*,IF(l.leavestatus = 'Approved', 'A', 'P') as status FROM `main_leaveplan` AS `l` WHERE (l.isactive = 1 AND l.user_id = '$id' and l.leavestatus IN(1,2))";
	...
	public function getManagerApprovedOrPendingLeavesData($id)
	{
		...
		$query = "SELECT `l`.*,IF(l.leavestatus = 'Approved', 'A', 'P') as status,u.userfullname
				 FROM `main_leaveplan` AS `l` left join main_users u on u.id=l.user_id 
	...
	public function getReportingManagerId($id)
	{
	    $result =  $this->select()
    				->setIntegrityCheck(false) 	
    				->from(array('l'=>'main_leaveplan'),array('repmanager'=>'l.rep_mang_id','hrmanager'=>'l.hr_id'))
 	  				->where("l.isactive = 1 AND l.id = ".$id);
	
    	return $this->fetchAll($result)->toArray();
	}
	
	public function SaveorUpdateLeaveplan($data, $where)
	{
	    if($where != '')
		{
		...
		{
			$this->insert($data);
			$id=$this->getAdapter()->lastInsertId('main_leaveplan');
			return $id;
		}
	}
	...
	public function getUserID($id)
    {
    	$result =  $this->select()
    				->setIntegrityCheck(false) 	
    				->from(array('l'=>'main_leaveplan'),array('l.user_id'))
 	  				->where("l.isactive = 1 AND l.id = ".$id);
	...
	public function checkdateexists($from_date, $to_date,$loginUserId)
	{
		...
		$query = "select count(l.id) as dateexist from main_leaveplan l where l.user_id=".$loginUserId." and l.leavestatus IN(1,2) and l.isactive = 1
    ...
    */	   
	...
	{	
		...
		if($objName == 'manageremployeevacations')
		{
		       if($searchData != '' && $searchData!='undefined')
                ...
                $tablecontent = $this->getEmployeeLeaveplan($sort, $by, $pageNo, $perPage,$searchQuery,$loginUserId);      				
				$dataTmp = array(
					'sort' => $sort,
					'by' => $by,
					'pageNo' => $pageNo,
	...
	public function getUsersAppliedLeaves($userId)
	{
		$result =  $this->select()
    				->setIntegrityCheck(false) 	
    				->from(array('l'=>'main_leaveplan'),array('l.from_date','l.to_date'))
	...
	public function checkLeaveExists($applied_from_date,$applied_to_date,$from_date, $to_date,$loginUserId)
	{
		...
		$query = "select count(l.id) as leaveexist from main_leaveplan l where l.user_id=".$loginUserId." and l.leavestatus IN(1,2) and l.isactive = 1
...
public function getLeaveDetails($id)
	{
		$result =  $this->select()
    				->setIntegrityCheck(false) 	
    				->from(array('l'=>'main_leaveplan_summary'),array('l.*'))
	...
	public function getLeavesCount($userid,$status='') {
        ...
        $query = "select count(*) cnt from main_leaveplan l 
