	...
	public function SaveorUpdateEmpTMEData($data, $where)
	{
		try{
		    if($where != ''){
				...
				$this->insert($data);
				$id=$this->getAdapter()->lastInsertId('tm_emp_timesheets_edited');
				return $id;
			}
		}
		catch(Exception $e)
...
//			  }else{
	...
	public function getTimesheetData($empId,$year,$month) {
			...
			$select = $this->select()
				  ...
				  ->joinLeft(array('pt'=>'tm_project_tasks'),'pt.id=et.project_task_id')
				  ->joinLeft(array('t'=>'tm_tasks'),'t.id=pt.task_id')
				  ->joinLeft(array('p'=>'tm_projects'),'p.id=pt.project_id');
	  	  	$select->where("et.emp_id = $empId and CAST(et.date AS DATE) BETWEEN '$firstDate' and '$lastDate'");
	  	  	// ->group('et.project_task_id');
	  	  	// echo $select;     
		return $this->fetchAll($select)->toArray();
	}
