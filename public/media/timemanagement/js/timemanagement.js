...
//===========================================================================================		
	/*	var mon = $('#mon_tot_hrs').text().trim();
          var res  = mon.split(":");
          var monmin = parseInt(res[0])*60;
          monmin  = monmin + parseInt(res[1]);

          var tue = $('#tue_tot_hrs').text().trim();
          var res  = tue.split(":");
          var tuemin = parseInt(res[0])*60;
          tuemin  = tuemin + parseInt(res[1]);

          var wed = $('#wed_tot_hrs').text().trim();
          var res  = wed.split(":");
          var wedmin = parseInt(res[0])*60;
          wedmin  = wedmin + parseInt(res[1]);

          var thu = $('#thu_tot_hrs').text().trim();
          var res  = thu.split(":");
          var thumin = parseInt(res[0])*60;
          thumin  = thumin + parseInt(res[1]);

          var fri = $('#fri_tot_hrs').text().trim();
          var res  = fri.split(":");
          var frimin = parseInt(res[0])*60;
          frimin  = frimin + parseInt(res[1]);



                if(sunHrs >= 8 || monmin >= 510 || tuemin >=  510 || wedmin >= 510 || thumin >= 510
                    || frimin > 270 || satHrs >= 8) {
                  
                   $("#error_message").show(); 
                   $('#error_message').empty();      
                   $("#error_message").html('<div id="messageData" class="ml-alert-1-error"><div style="display:block;"><span class="style-1-icon error"></span>Time per day cannot be more than 8:30 Hours and Time per day for friday cannot be more than 4:30</div></div>');
*/
//===========================================================================================	
  	
              	if(sunHrs >= 8.5 || monHrs >= 8.5 || tueHrs >= 8.5 || wedHrs >= 8.5 || thuHrs >= 8.5
              			|| friHrs >= 4.5 || satHrs >= 8.5) {
...
//	console.log(" fri "+friHrs);
	...
	//===================CODE BIGIN HERE added by melese===============================================================
/*	var mon = $('#mon_tot_hrs').text().trim();
    var res  = mon.split(":");
    var monmin = parseInt(res[0])*60;
    monmin  = monmin + parseInt(res[1]);
